...
def test_membrane_candidate_on_high_direct_mismatch() -> None:
    # Force a path with high direct mismatch by disabling buffers and using high-misfit film
    cand = StructureCandidate(
        formula="CrN",
    ...
    score = score_si_feasibility(cand)
    # Depending on exact mismatch may or may not trigger; at least note is present
    assert score.membrane_transfer_note
