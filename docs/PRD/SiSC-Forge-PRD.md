...
| Phase 3 partial | DFT+U + `DFTUResult` (P3.1); Wannier prep + quality metrics + `ready_for_dmft` + automated nscf/pw2wannier90 (P3.2 / P3.2.1); `DMFTResult` scaffold + controlled solid_dmft launcher (model + gate + mock + drop-in parser + toml/run package + optional invoke — production U/J/β residual) (P3.3); pairing eigenvalue → common `performance_score` with `epw_then_dmft` precedence (P3.4); oxygen-vacancy / infinite-layer enumeration (P3.5); mixed AL pools (P3.6) |
...
- **Shipped (P3.1–P3.6):** DFT+U + `DFTUResult`; Wannier prep + `ready_for_dmft` gate + P3.2.1 automated nscf/pw2wannier90; `DMFTResult` scaffold + controlled launcher (model + gate + mock + drop-in parser + toml/run package + optional invoke); pairing eigenvalue → common `performance_score` with documented precedence; oxygen-vacancy / infinite-layer enumeration for nickelates; mixed conventional/unconventional AL acquisition (`off` / `joint` / `separate`).
- **Residual:** production CTHYB calibration / solid_dmft version matrix; production GNN λ/Tc heads (ALIGNN/MatGL); material-specific production Wannier libraries; NdNiO₂ science golden.
