...
**Still residual:** production CTHYB calibration (U/J/β) and solid_dmft version matrix, production ALIGNN/MatGL λ/Tc heads, golden NdNiO₂ science campaign. Native solid_dmft h5/dat → `DMFTResult` is shipped (JSON drop-in still preferred).
...
| `examples/mgb2_epw.yaml` | MgB₂ EPW golden (`dft.nosym` + `ph_search_sym: false`) |
