...
def _lookup_elastic(
    ...
    # Unsupported materials: do not invent elastic params. Callers fall back
    # to the mismatch-only thickness band and label the method "heuristic fallback".
    # (Providing a synthetic ν=0.25 would otherwise claim Matthews–Blakeslee.)
    return None
...
def matthews_blakeslee_hc_nm(
    ...
    *,
    ...
    nu = float(poisson)
    # prefactor = b * (1 - ν cos²α) / (8 π f (1 - ν) cos λ)
    cos_l = max(abs(float(cos_lambda)), 1e-6)
    prefactor = (b_nm * (1.0 - nu * cos2_alpha)) / (
        8.0 * math.pi * f * max(1.0 - nu, 1e-6) * cos_l
    )
...
def _band_from_hc(hc_nm: float | None, mismatch_pct: float | None) -> float:
    """Map h_c (or mismatch) to a recommended thickness (nm).

    When a physical h_c is available, recommend ~0.6 × h_c (process margin).
    No artificial floor is applied on top of a physical coherent limit — a
    fabrication minimum should be surfaced separately if needed. Fallback
    mismatch bands still use conservative discrete values.
    """
    if hc_nm is not None and hc_nm > 0:
        # Recommend ~0.6 of coherent limit; keep a tiny positive floor only
        # for numerical sanity, never above the physical h_c.
        return float(min(max(hc_nm * 0.6, 0.1), 200.0))
...
def membrane_transfer_heuristic(
    *,
    ...
    # 6% path rule is documented as a multilayer residual-mismatch trigger.
    # Gate it so single-buffer / direct paths use only the 8% direct threshold.
    if is_multilayer and pmm is not None and pmm >= 6.0:
        candidate = True
        reasons.append(f"path effective |mismatch| {pmm:.1f}% ≥ 6% (multilayer)")
