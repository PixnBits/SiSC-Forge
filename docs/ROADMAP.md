...
- **Interleaved active learning**: prioritise → calculate → promote → retrain in short cycles; do not wait for a large static label set before the first useful surrogate.
...
- Active-learning loop (uncertainty sampling or simple UCB) that prioritizes expensive jobs (**prioritisation shipped; lightweight retrain + seed management in residual / Phase 1.5**).
...
**Exit criteria**: Automated recovery of MgB₂ Tc; AL prioritisation working; 50–200 candidate nitride campaign can be driven with intelligent prioritization on modest resources; AL bootstrap milestone (seed + first surrogate + one complete cycle) demonstrated.
...
| 1     | Conventional EPW + AL prioritisation | 6–10 weeks     | Workstation + small cluster |
